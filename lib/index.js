...
weSendEmail.init = function init(configs){
  if(configs){
...
weSendEmail.getEmailTemplateDir = function getEmailTemplateDir(){
  ...
  if(sails.config.themes.enabled){
    var themeEnabled = sails.config.themes.themeEnabled;

    templatesDir = sails.config.appPath + '/node_modules/'+ sails.config.themes.enabled + '/' + themeEnabled.configs.emailTemplates.path;
  }else{
